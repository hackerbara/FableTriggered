...
## Phase 2 — Cutover & rename (new clean folder) — ✅ COMPLETE 2026-07-04

**Done.** `/Users/MAC/Documents/harnessmonkey`, single root commit `8c69d42` "HarnessMonkey v1" (218 files), no remote. All four sweeps verbatim-empty (independently re-verified from the main session). Suite in new tree: 924 passed / 3 skipped (skips = opt-in `.development` artifact tests, correctly absent). MIT LICENSE in. `ba59b0f` (capy/dragons fix) re-synced mid-flight and renames re-applied on fresh content with real emitter regeneration + parity. Capy Enter bug: FIXED on main by the user's grinding agent, README known-issue note removed.
**Post-launch polish noted (not blockers):** `hidden-context-inline` ⟷ `hidden-context-drawer` and the two art scenes conflict only in prose/anchors, not as `conflictsWithPackages` manifest data — declare both pairs for GUI-level warnings in a post-launch round.
**Remaining to publish:** GIFs (capture prompt: `docs/superpowers/plans/2026-07-04-gif-capture-prompt.md`) → copy into new tree's `assets/demos/` → amend `8c69d42` → image-eyeball gate (4.2a) → user adds remote + pushes (4.3) → fresh-clone proof (4.4). PyPI already claimed (`harnessmonkey` 0.0.1 live).
...
5. **OPEN, SHIPS AS KNOWN ISSUE — capybara-onsen breaks Enter-to-open on drawers.** First fix attempt (context-value memoization, `9d31dc2`/`afd0c6d`) was plausible, parity-green, and **user-verified insufficient**. Full evidence + ranked hypotheses + constraints: `docs/superpowers/reviews/2026-07-04-capy-drawer-enter-handoff.md` — the user is assigning a separate agent to grind it. v1 ships with a README known-issue note (done). **GIF plan adjusted (user decision):** drawers recorded WITHOUT capy; capy recorded solo; no longer blocks the capture session.
6. Launch posture (user decisions): LaunchAgent ships (TCC fix `5c36509` — dedicated `~/.claude-monkey/app` venv, verified no PermissionError) + README manual-launch fallback line for the macOS approval gate. Tray icons moved into the package (invisible-monkey fix — icons were repo-relative and absent from the app venv).
7. Suite note: full-suite failures on build-heavy tests with passes-in-isolation = disk pressure; clear `.development/claude-monkey-builds/` (safe — active build lives in `~/.claude-monkey/versions/`). Suite green at 923/2/0 after cleanup.
