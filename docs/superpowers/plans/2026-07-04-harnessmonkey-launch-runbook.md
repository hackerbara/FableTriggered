...
- **G1 regression repair agent in flight**: the merge left 7 stale-expectation failures in `tests/test_reference_packages.py` (old payload filenames pre-migration). Agent updating expectations to merged reality. Target end state: suite = 1 failed (known: `test_dvd_cursor_goblin.py`, pre-existing, package is cut in 1.1) + rest green.
