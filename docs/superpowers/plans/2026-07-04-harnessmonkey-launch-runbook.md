...
- [ ] 1.7a **`add-all` command (user decision 2026-07-04):** new CLI subcommand copying every package under the repo's `packages/` into the state dir via the existing `add_package` validation path (per-package success/failure report, non-zero exit if any fail). The install story documents it (README already does); packages stay disabled until `enable-patch`. Include CLI test. Note: resolution stays state-dir-only (`test_no_repo_package_source_v3.py` guards this deliberately — `add-all` copies, it does not make the repo a live source).
