...
Usage:
    uv run python examples/capybara-onsen-generator/translate_to_windows.py \
        ...
        --win-source ~/.local/share/claude-monkey-dev/win32-x64/2.1.201/claude.exe
...
from claude_monkey.bun_graph import parse_bun_section  # noqa: E402
from claude_monkey.binary_format import locate_bun_section  # noqa: E402
...
def main() -> None:
    ...
    ap.add_argument(
        "--win-source",
        default=str(Path.home() / ".local/share/claude-monkey-dev/win32-x64" / VERSION / "claude.exe"),
    )
    args = ap.parse_args()
