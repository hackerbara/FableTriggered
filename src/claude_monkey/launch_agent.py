...
APP_DIR_NAME = "app"
APP_MARKER_NAME = ".claude-monkey-app"
...
def app_venv_dir(state_dir: Path) -> Path:
    ...
    PermissionError on pyvenv.cfg. ~/.claude-monkey is not TCC-protected."""
    return Path(state_dir) / APP_DIR_NAME


def app_gui_executable(state_dir: Path) -> Path:
    return app_venv_dir(state_dir) / "bin" / "claude-monkey-menubar"
...
def provision_app_venv(repo_root: Path, state_dir: Path, runner=run_command) -> Path:
    ...
    (venv_dir / APP_MARKER_NAME).write_text(
        "Managed by `claude-monkey install`. Safe to delete; "
...
def _looks_like_our_app_dir(venv_dir: Path) -> bool:
    ...
    if (venv_dir / APP_MARKER_NAME).exists():
        return True
    if (venv_dir / "bin" / "claude-monkey-menubar").exists():
        return True
    return False
...
def uninstall_agent(home: Path, runner=run_command) -> CommandResult:
    ...
    app_dir = app_venv_dir(Path(home) / ".claude-monkey")
...
def gui_executable() -> Path:
    ...
    claude-monkey-menubar` runs fine here because Terminal.app holds its own
