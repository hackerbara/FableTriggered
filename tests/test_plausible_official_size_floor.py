...
from claude_monkey.status import classify_plausible_official_source, status_payload
...
# a candidate that is executable and not one of ClaudeMonkey's own managed
...
def write_small_wrapper(path: Path, text: str = "#!/bin/sh\necho cmux-wrapper\n") -> Path:
    ...
    executable, non-ClaudeMonkey-managed file -- exactly the shape that
...
def test_classify_refuses_cmux_sized_wrapper(tmp_path):
    paths = StatePaths(tmp_path / ".claude-monkey")
