...
from claude_monkey import install as install_module
...
from claude_monkey.repair import repair_shim_action
...
# over a bare ClaudeMonkey shim within ~15s of any fresh official-claude
...
def test_reinstall_over_locked_shim_relocks_if_swap_fails_after_unlock(tmp_path, monkeypatch):
    ...
    existing ClaudeMonkey-locked shim and then the swap itself fails, the
    ...
    def failing_replace(self, dest):
        if self.name.endswith(".claude-monkey.tmp"):
